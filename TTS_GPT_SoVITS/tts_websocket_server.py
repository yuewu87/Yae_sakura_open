...
# 导入配置和推理器
from tts_config import TTS_CONFIG, setup_directories, validate_model_files
...
class TaskManager:
    ...
    def update_task_status(self, task_id: str, status: str, **kwargs):
        ...
        if task_id in self.tasks:
            ...
            logger.debug(f"任务 {task_id[:8]} 状态更新为: {status}")
...
class TTSServer:
    ...
    def __init__(self, host: str = None, port: int = None):
        ...
        logger.debug(f"TTS服务器初始化完成，监听地址: {self.host}:{self.port}")
    ...
    async def _handle_synthesize_speech(self, client_id: str, websocket: WebSocketServerProtocol, data: dict):
        ...
        if active_tasks >= TTS_CONFIG["max_concurrent_tasks"]:
            self._pending_queue.append((client_id, data))
            await self.client_manager.send_to_client(client_id, {
                "type": "task_queued", "position": len(self._pending_queue),
            })
            return
    ...
    def _process_next_pending(self):
        if not self._pending_queue:
            return
        client_id, data = self._pending_queue.pop(0)
        ws = self.client_manager.clients.get(client_id)
        if ws is None:
            return
        asyncio.create_task(self._handle_synthesize_speech(client_id, ws, data))
