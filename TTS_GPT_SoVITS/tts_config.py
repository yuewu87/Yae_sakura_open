...
TTS_CONFIG = {
    # 模型/音频路径（相对于项目根目录）
    "live2d_path": os.path.join(BASE_DIR, "..", "data", "live2d_models", "八重樱"),
    "gpt_model_path": os.path.join(BASE_DIR, "..", "data", "TTS_models", "GPT_weights_v2", "八重樱-e10.ckpt"),
    "sovits_model_path": os.path.join(BASE_DIR, "..", "data", "TTS_models", "SoVITS_weights_v2", "八重樱_e10_s200.pth"),
    "default_ref_audio_path": os.path.join(BASE_DIR, "..", "data", "reference", "此时此刻，唯有你和我，共赏这轮明月。.wav"),
    ...
    "model_params": {
        ...
        "speed_factor": 1.0,
        "seed": -1,
        "is_half": True,              # FP16推理（True=更快/省显存, False=更高质量）
        "repetition_penalty": 1.1,   # 重复惩罚（1.0=不惩罚, 官方默认1.0-1.2，过高引入杂音）
        "sample_steps": 32,           # V3/V4模型采样步数
    },
    
    # 性能配置
    "max_text_length": 1000,  # 最大文本长度
    "max_concurrent_tasks": 4,  # 最大并发任务数（GPU内存限制）
