...
class TTSWebSocketClient:
    ...
    def __init__(self, 
                 server_url: str = "ws://localhost:8770",
                 http_base_url: str = "http://localhost:8005",
    ...
    def synthesize_speech(self, 
        ...
        if len(self.pending_tasks) > 100:
            old_ids = sorted(self.pending_tasks.keys())[:50]
            for oid in old_ids:
                if self.pending_tasks[oid]["status"] in ("completed", "failed"):
                    self.pending_tasks.pop(oid, None)
                    self.task_callbacks.pop(oid, None)
